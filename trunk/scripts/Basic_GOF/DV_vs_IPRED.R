...
if (file.exists (paste("pirana_temp/plot_DV_IPRED_",names(models)[1],".pdf", sep=""))){
    file.remove (paste("pirana_temp/plot_DV_IPRED_",names(models)[1],".pdf", sep=""))
}
pdf (file = paste("pirana_temp/plot_DV_IPRED_",names(models)[1],".pdf", sep=""))
for (i in 1:length(model_names)) {
    ...
    if (file.exists (tab_file)) {
        ...
        plot (x=tab$IPRE, tab$DV, main = paste (model_names[i],": ", mod$description, sep=""),
              pch=19, xlab="Individual prediction", ylab="Dependent variable")
    }
}
dev.off()

print (paste("#", "PIRANA_OUT ","pirana_temp/plot_DV_IPRED_",names(models)[1],".pdf", sep=""))
