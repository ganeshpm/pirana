...
if (file.exists (paste("pirana_temp/plot_DV_PRED_TIME_",names(models)[1],".pdf", sep=""))){
    file.remove (paste("pirana_temp/plot_DV_PRED_TIME_",names(models)[1],".pdf", sep=""))
}
pdf (file = paste("pirana_temp/plot_DV_PRED_TIME_",names(models)[1],".pdf", sep=""))
for (i in 1:length(model_names)) {
    ...
    if (file.exists (tab_file)) {
        ...
        print (
            xyplot (DV+PRED~TIME, data=tab,
                main = paste (model_names[i],": ", mod$description, sep=""),
                pch=19, xlab="Time", ylab="Dependent variable / Pop. prediction"
            )
        )
    }
}
...
print (paste("#", "PIRANA_OUT ","pirana_temp/plot_DV_PRED_TIME_",names(models)[1],".pdf", sep=""))
quit()
