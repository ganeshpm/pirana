...
### Description: Creates wres.dist.hist() using Xpose

models <- #PIRANA_IN
library(xpose4)

dir.create("pirana_temp")
pdf(paste("pirana_temp/","xpose_dist_wres_",names(models)[1],".pdf", sep=""))
for (i in 1:length(names(models))) {
    ...
    print(wres.dist.hist(xpdb))
}
dev.off()

print (paste("#", "PIRANA_OUT ","pirana_temp/xpose_dist_wres_",names(models)[1], ".pdf", sep=""))
