...
### Description: Creates wres.vs.pred() using Xpose

models <- #PIRANA_IN
library(xpose4)

dir.create("pirana_temp")
pdf(paste("pirana_temp/","xpose_wres_vs_pred_",names(models)[1],".pdf", sep=""))
for (i in 1:length(names(models))) {
    ...
    print(wres.vs.pred(xpdb))
}
dev.off()

print (paste("#", "PIRANA_OUT ","pirana_temp/xpose_wres_vs_pred_",names(models)[1], ".pdf", sep=""))
