...
### Description: Creates parm.splom() using Xpose

models <- #PIRANA_IN
library(xpose4)

dir.create("pirana_temp")
pdf(paste("pirana_temp/","xpose_parm_splom_",names(models)[1],".pdf", sep=""))
for (i in 1:length(names(models))) {
    ...
    print(parm.splom(xpdb))
}
dev.off()

print (paste("#", "PIRANA_OUT ","pirana_temp/xpose_parm_splom_",names(models)[1], ".pdf", sep=""))
